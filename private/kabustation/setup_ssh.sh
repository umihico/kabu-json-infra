...
aws s3 cp s3://kabu-data-bucket/.ssh/config.d/kabu-json-windows ~/.ssh/config.d/kabu-json-windows
ssh-keygen -R kabu-json-windows # IPが変わった場合に、~/.ssh/known_hostsから古い方を削除して、Man in the Middle Attack警告を不要に出さない
aws s3 cp s3://kabu-data-bucket/.ssh/config.d/kabu-json-linux ~/.ssh/config.d/kabu-json-linux
